...
def artifacts_parser(header: list, lst_of_lst: list):
...
def stats_parser(header: list, lst_of_lst: list):
    lst = ast.literal_eval(lst_of_lst)
...
class Artifact(Resource):
    ...
    def get(self):
        try:
            ...
            for dicts in result:
                new_lst = (
                    artifacts_parser(headers, dicts["setEffect"])
                    if dicts["setEffect"]
                    else None
                )
    ...
    def get(self):
        try:
            ...
            if name:
                name = unquote(request.args["name"]).replace("_", " ")
                result = query(
                    f" SELECT * FROM characters WHERE characterName = ?", name
                )
                lst = result
    ...
    def get(self):
        try:
            ...
            if name and weapon_type:
                ...
                result = query(
                    f" SELECT * FROM weapons WHERE weaponType = ? AND weaponName = ?",
                    request.args["type"],
                    weapon_name,
                )
                lst = result
                ...
                if not lst:
                    return {
                        "Error": "Name or weapon type does not exist for weapon."
                    }, 400
            ...
            if weapon_type:
                result = query(
                    f" SELECT * FROM weapons WHERE weaponType = ?",
                    request.args["type"],
                )
                headers = ["Level", "Attack", "Secondary Stats"]
                valid_weapon_types = [
                ...
                if not result:
                    return {
                        "Error": f"Weapon type does not exist for weapon. Valid weapon types are: {str(valid_weapon_types)}"
                    }, 400
